...
from llava.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, POSE_TOKEN_INDEX
...
class LlavaMetaModel:
    ...
    def initialize_pose_modules(self, model_args, fsdp=None):
        ...
        if self.get_pose_tower() is None:
            pose_tower = build_pose_tower(model_args)
...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal_pose(
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            cur_labels = labels[batch_idx]
            cur_labels_nopose = []
            for i in range(len(pose_token_indices) - 1):
